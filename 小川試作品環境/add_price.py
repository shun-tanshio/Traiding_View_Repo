...
def calculate_rsi_ma(df_price: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    """
    株価データからRSIとRSI MA20を計算
    入力: df_price（Tickerがindex、日付が列）
    出力: (rsi, rsi_ma20)
    """
    # 転置（行=日付、列=銘柄）
    df_t = df_price.T
    ...
    # 元の形式に戻す（Tickerがindex、日付が列）
    rsi_out = rsi.T
    rsi_ma20_out = rsi_ma20.T

    return rsi_out, rsi_ma20_out
...
def main():
    ...
    # ===== RSI・MA20を計算して保存 =====
    print("\n【計算中】RSI・MA20を計算中...")
    rsi_out, rsi_ma20_out = calculate_rsi_ma(merged)

    rsi_out.to_csv("nikkei225_RSI.csv", encoding="utf-8-sig")
    rsi_ma20_out.to_csv("nikkei225_RSI_MA20.csv", encoding="utf-8-sig")

    print("【保存】nikkei225_RSI.csv を生成しました")
    print("【保存】nikkei225_RSI_MA20.csv を生成しました")
