...
def fetch_close_open_wide_1y(tickers: list[str]) -> tuple[pd.DataFrame, pd.DataFrame, list[str]]:
    ...
    for i in range(0, len(tickers), CHUNK_SIZE):
        ...
        try:
            ...
            print("【株価取得エラー】'Close' または 'Open' 列が取得できませんでした。")
            print(f"  エラー内容: {repr(e)}")
            failed.extend(chunk)
            continue
        ...
        close_wide = close.T
        open_wide = open_price.T

        # 全部NaNのTickerも失敗扱い
        all_nan = close_wide.isna().all(axis=1)
        nan_failed = close_wide.index[all_nan].tolist()
        if nan_failed:
    ...
    close_out = close_out.sort_index().reindex(sorted(close_out.columns), axis=1)
    ...
    open_out = open_out.sort_index().reindex(sorted(open_out.columns), axis=1)

    return close_out, open_out, sorted(set(failed))

def main():
    tickers = parse_tickers(TICKERS_TEXT)
    print(f"対象ティッカー数: {len(tickers)}")

    close_wide, open_wide, failed = fetch_close_open_wide_1y(tickers)
    ...
    # 始値も保存
    open_csv = "prices_open_wide.csv"
    open_wide.to_csv(open_csv, encoding="utf-8-sig", float_format="%.2f")
    print(f"【保存】CSVを保存しました: {open_csv}")
